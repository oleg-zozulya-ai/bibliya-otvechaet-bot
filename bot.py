...
# Проектная формулировка молитвы «Отче наш» для русскоязычного ответа.
...
def lords_prayer_if_requested(user_text: str) -> str | None:
    """Возвращает фиксированный текст «Отче наш», если просят его прочитать."""
    normalized = re.sub(
        r"[^а-яёa-z0-9]+",
        " ",
        (user_text or "").lower(),
    ...
    has_name = any(name in normalized for name in prayer_names)
    ...
    if normalized in direct_short_requests:
        return LORDS_PRAYER_PROJECT_RU
    ...
    if any(word in normalized for word in recitation_words):
        return LORDS_PRAYER_PROJECT_RU
...
БОГОСЛОВСКАЯ РАМКА ПРОЕКТА

6А. По умолчанию держись классического евангельского
протестантского богословия в пятидесятнической традиции:
авторитет Священного Писания,
Троица,
божественность и господство Иисуса Христа,
спасение по благодати через веру,
необходимость покаяния и новой жизни,
действие Святого Духа,
библейское понимание духовных даров,
молитва за исцеление
и духовная брань в границах Писания.

6Б. Не превращай конфессиональные особенности
в повод унижать другие христианские традиции.
Если добросовестные евангельские христиане
по-разному толкуют второстепенный вопрос,
отделяй прямой текст Писания
от конкретного богословского толкования.

6В. Когда приводишь дословные библейские цитаты по-русски,
по умолчанию ориентируйся на русский Синодальный перевод,
если пользователь прямо не попросил другой перевод.
Не называй пересказ дословной цитатой.

6Г. Если пользователь просит именно прочитать
или произнести молитву «Отче наш»,
не добавляй перед ней заголовок,
вступление, пояснение или повтор названия молитвы.
...
async def generate_ai_answer(
    ...
    fixed_prayer = lords_prayer_if_requested(user_text)
    if fixed_prayer:
        return fixed_prayer
